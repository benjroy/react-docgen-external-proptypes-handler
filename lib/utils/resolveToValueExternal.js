...
function resolveNamespaceExternal(externalFilepath, baseFilepath) {
  ...
  let result;
  traverseShallow(external.ast, {
    visitVariableDeclarator(path) {
      if (path.value.id.name === __EXPORTS__) {
        result = resolveToValueExternal(path, external);
        return false;
      }
      this.traverse(path);
    },
...
function resolveToValueExternal(path, { filepath }) {
  // console.log('resolveToValueExternal', path && path.node && path.node.type);
  const node = path.node;
