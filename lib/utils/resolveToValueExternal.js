      ...
      if (isExportsOrModuleAssignment(path)) {
        ...
        const keys = getMembers(expressionPath.get('left')).reduce(
          (memo, { path, computed, argumentsPath }) => {
            // console.log('member', path.node.name);
        ...
        // console.log('visitExpressionStatement, isExportsOrModuleAssignment', keys);
...
/*
    visitAssignmentExpression(path) {
      ...
      while (n.MemberExpression.check(leftPath.node)) {
        // console.log('visitAssignmentExpression leftPath MemberExpression', leftPath);
        if (
          (leftPath.node.object.name === 'module'
          && leftPath.node.property.name === 'exports')
          || leftPath.node.object.name === 'exports'
        ) {
          // TODO: can this be cleaned?
          if (n.MemberExpression.check(leftPath.parentPath.node)) {
            // console.log('found exports', valuePath);
            const key = leftPath.parentPath.get('property').node.name;
            keys.push(key);
          // } else if (n.AssignmentExpression.check(leftPath.parentPath.node)) {
          //   console.log('found exports in AssignmentExpression', leftPath.parentPath.get('right'));
          }
...
*/
    ...
    visitExportDefaultDeclaration(path) {
      ...
      // console.log('visitExportDefaultDeclaration', node.declaration);
      // if (
      //   !n.Identifier.check(node.declaration) &&
      //   n.Identifier.check(node.declaration.id)
      // ) {
      ...
      //   // // inject assignment for this default export value;
      //   // const identifier = b.identifier(__DEFAULT__);
      ...
      //   // valuePath = { node: identifier };
      // }

      // if (n.ClassDeclaration.check(valuePath.node)) {
      //   valuePath = valuePath.get('id');
      // // } else {
  ...
  const namespaceObjectExpression = (keyValuePaths) =>
    b.objectExpression(
      keyValuePaths.map(([key, valuePath]) => {
        console.log('key', key, valuePath);
        return b.objectProperty(b.identifier(key), valuePath.node)
      }
      )
    );
  ...
  // console.log('code', recast.print(external.ast).code);
...
function findLastAssignedValueExternal(scope, name, { ast, filepath }) {
  ...
  traverseShallow(scope.path.node, {
    ...
    //     return this.traverse(path);
    //   }
    //   result = path.get('declaration').get('body');
    //   // throw "-------";
    //   return false;
    // }
  });
...
function resolveToValueExternal(path, { ast, filepath }) {
  console.log('resolveToExternalValue', path.node && path.node.type, filepath);
  if (path.path) {
  ...
  if (n.VariableDeclarator.check(node)) {
    ...
    if (
      ...
      const calleeName = getNameOrValue(path.get('callee'));
      if ('require' === calleeName) {
        // console.log('require(...)')
        const externalFilepath = resolveToModule(path);
        // console.log('externalFilepath', externalFilepath);
        const external = resolveNamespaceExternal(externalFilepath, filepath);
      ...
      // } else if ('_interopRequireDefault' === calleeName) {
      //   // console.log('_interopRequireDefault', node);
      //   // console.log('2 _interopRequireDefault', path.get('arguments').get(0));
      //   const external = resolveToValueExternal(path.get('arguments').get(0), { ast, filepath });
      //   console.log('_interopRequireDefault EXTERNAL', external.filepath, external.path.node);
      //   return external;


      //   const valuePath = getMemberValuePath(external.path, 'default');
      //   // console.log('_interopRequireDefault EXTERNAL', external.path.node);
      //   // console.log('_interopRequireDefault EXTERNAL', valuePath.node);
      //   // return external;
      //   const resolved = resolveToValueExternal(valuePath, external);
      //   // console.log('_interopRequireDefault EXTERNAL', resolved.filepath, recast.print(resolved.path.node).code);
      //   console.log('_interopRequireDefault EXTERNAL', resolved.filepath, resolved.path.node);

      //   return resolved;
      //   throw 'interopRequireDefault'; 
      }
    ...
    console.log('resolved MemberExpression', resolved.path.node.type);
    if (n.ObjectExpression.check(resolved.path.node)) {
      ...
      for (const propertyName of toArrayExternal(path, { ast, filepath }).slice(1)) {
        console.log('for (const propertyName of toArray(path).slice(1))', propertyName);
        ...
        console.log('returning resolved property path', propertyPath);
        // propertyPath = resolveToValueExternal(propertyPath, resolved);
    ...
    // } else if (n.FunctionExpression.check(resolved.path.node)) {
    //   console.log('MemberExpressionRoot resolved to FunctionExpression', recast.print(resolved.path.node).code);

    //     console.log('333 FunctionExpression component?', {
  ...
  //   return resolveToValueExternal(resolved.path, resolved);
    ...
    if (
      ...
      // throw new Error('identifier early return');
    ...
    if (scope) {
      ...
      // if (!resolvedPath) {
        ...
        // console.log('if (!resolvedPath) end resolved', resolved);
