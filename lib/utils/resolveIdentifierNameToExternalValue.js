...
function resolveIdentifierNameToExternalValue(variable, ast, filepath) {
  ...
  types.visit(ast, {
    ...
    visitImportNamespaceSpecifier: function(path) {
      ...
      if (path.value.local.name === variable) {
        console.log('2 visitImportNamespaceSpecifier', variable);
    ...
    visitIdentifier(path) {
      ...
      if (path.value.name === variable) {
        ...
        while (extKeyNames.length) {          
          ...
          console.log('INSIDE', keyName)
          // const propPath = targetPath.get('properties').get(keyName);
          const propPath = targetPath.get('properties').filter(
            propertyPath => {
              // console.log('propertyPath', propertyPath);
              return keyName === getNameOrValue(propertyPath.get('key'))
            }
          );
          // console.log('propPath', propPath);
          targetPath = propPath[0].get('value');
          // targetPath = resolveIdentifierNameToExternalValue(keyName, targetPath, targetFilepath)
        }

        console.log('2 TARGET PATH FOUND', variable, targetPath.node.type);
