...
// TODO: name this resolveIdentifierToValue
function findIdentifier(variable, ast, filepath) {
  console.log('findIdentifier', variable);
  var result = false;
  ...
  types.visit(ast, {
    visitImportSpecifier: function(path) {
      if (path.value.local.name === variable) {
        ...
        result = findIdentifier(extVariable, extAst, extFilepath);
        return false;
      }
      this.traverse(path);
    },
    ...
    visitExportDefaultDeclaration: function(path) {
      if (variable === 'default') {
        ...
        if (n.Identifier.check(path.node.declaration)) {
          ...
          result = findIdentifier(path.node.declaration.name, ast, filepath);
          return false;
        }
